 ...
 */
public class HoverMouse implements HoverMouseInterface { //Hovermouse som har ett interface för säkerthetens skull
    ...
    @Override
    public void inHoverSize(Button button, int ID, ListView list, Label fel) { //jobbar med shoppen
        button.setOnMouseEntered((MouseEvent e) -> {
            button.setScaleX(2);
            button.setScaleY(2);
            ...
            try {
                ...
                if (ID >= 1 && ID <= 3) {   //OM DET ÄR ETT VAPEN MED ANDRA ORD
                    ResultSet rs = DBConnect.CreateSelectStatement("select * from weapon where weapontype = '" + getclass + "';", fel);
                    while (rs.next()) {
                    ...
                    ifArmorOrWeapon(ID, list, fel);

                } else if (ID >= 4 && ID < 7) { // ARMOR
                    ResultSet rs = DBConnect.CreateSelectStatement("select * from armor where armortype = '" + getclass + "';", fel);
                ...
                fel.setText("Error loading items");
            } finally {
                DBConnect.close(fel);
            }
        });
    ...
    @Override
    public void buyWeaponsAdd(ListView list, int index, Label fel) { //jobbar med shoppen
        ...
        try {
            ...
            fel.setText("Error loading weapon stats");
        }
    }

    @Override
    public void buyArmorsAdd(ListView list, int index, Label fel) { //jobbar med shoppen
        ...
        try {
            ...
            fel.setText("Error loading armor stats");
        }
    }

    @Override
    public void ifArmorOrWeapon(int ID, ListView list, Label fel) { //jobbar med shoppen

        if (ID == 1) {
            buyWeaponsAdd(list, 0, fel);

        } else if (ID == 2) {

            buyWeaponsAdd(list, 1, fel);
        } else if (ID == 3) {

            buyWeaponsAdd(list, 2, fel);
        } else if (ID == 4) {

            buyArmorsAdd(list, 0, fel);
        } else if (ID == 5) {

            buyArmorsAdd(list, 1, fel);
        } else if (ID == 6) {

            buyArmorsAdd(list, 2, fel);
        }
    }
